...
from models.clip_lora2.slinet_lora import SliNetLora
from peft import get_peft_config, get_peft_model, LoraConfig, TaskType



class LoraPrompt(BaseLearner):

    def __init__(self, args):
        super().__init__(args)

        if args["net_type"] == "lora_prompt_clip":
            self._network = SliNetLora(args)
    ...
    def _train(self, train_loader, test_loader):
        ...
        for name, param in self._network.named_parameters():
            ...
            #     param.requires_grad_(True)
            #     paramGrad+=param.numel()
            # lora_image_model_pool.4.base_model.model.transformer.resblocks.1.attn.out_proj.lora_A.weight', 
            # 'lora_image_model_pool.4.base_model.model.transformer.resblocks.0.attn.out_proj.lora_A.weight',
            # 'lora_image_model_pool.3.base_model.model.transformer.resblocks.11.attn.out_proj.lora_A.weight', 
            # 'lora_image_model_pool.2.base_model.model.transformer.resblocks.4.attn.out_proj.lora_B.weight', 
            if "lora_image_model_pool" + "." + str(numtask - 1) in name:
                if "lora_A" in name:
                    param.requires_grad_(True)
                    paramGrad+=param.numel() 
                if "lora_B" in name:
